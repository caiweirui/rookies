        ...
        *flaginput = 0;
        ...
        while(1)
        {
            if(x < 1 || x > row || y < 1 || y > col)
            {
                ...
                if(*flaginput <= 3)
                {
                    printf("\nPrince and Princess, you have entered wrong, please re-enter:");
                    scanf("%d %d", &x, &y);
                }
                else
                {
            ...
            *flaginput = 0;
            while(1)
            {
                ...
                else
                {
                    ...
                    if(*flaginput <= 3)
                    {
                        printf("\nPrince and Princess, the place you entered has been occupied, please re-enter:");
                        scanf("%d %d", &x, &y);
                    }
                    else
                    {
...
void who_win(char board[ROW][COL], int row, int col, int *flagwin)
{
    ...
    if(pw)
    {
        printf("\nPrince and Princess perform incredibly!\n\n\n");
        *flagwin = 1;
    }
    else if(cw)
    {
        printf("\nGo home, child. This game is not suitable for you.\n\n\n");
        *flagwin = 1;
    }
    else
    {
...
void game()
{
    ...
    //下棋
    printf("\nPrince and Princess, please play chess!\n");
    printf("Enter the number of rows and columns, for example: 1 1, which is located in the first row and first column:\n");
    while(!flagwin)
    {
        //玩家下棋
...
int main()
{
    ...
    int countinput = 0;
    printf("\n\nDoes the prince and princess want to play three pieces of chess?\n");
    printf("Enter \"1\" to play the game, enter \"0\" to exit the game:");
    int flag = 0;
    do
    {
        scanf("%d", &flag);
    ...
    {
        game();
        printf("\nDoes the princess and prince still want to continue?\n");
        printf("Enter \"1\" again, enter \"0\" to exit the game:");
        int reinput = 0;
        int reflag = 0;
        for(reflag = 0; reflag < 3; reflag++)
        {
            ...
            if(1 == reinput)
            {
                printf("\nDear Princess and Prince, you have chosen to have another one\n\n\n");
                break;
            }
            else if(0 == reinput)
            {
